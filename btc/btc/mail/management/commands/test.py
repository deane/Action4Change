...
To unsubscribe from these emails <a href="http://challenges4change.org/mail/un/?mail='+email+'">click here</a>\
        ...
        rems =['dean@hearplanet.com']
        for r in rems:
            print('sending to : '+r)
            mail = urllib2.quote(r)
            req = urllib2.Request('\
http://sendgrid.com/api/mail.send.json?to='+mail+'&from='+FROM+'&fromname=Challenges%204%20Change&subject=[test]'+SUBJECT+'&html='+MESSAGE(tdate, today, mail)+'&api_user=dean&api_key=dean1')
